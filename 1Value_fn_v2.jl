...
VF(n) = -value_fn(n, sd, sf, shockgrid[eta,1], shockgrid[eta,2], shockgrid[eta,3], pd, pf, parameters, sdgrid, sfgrid, vinit)                     
...
#vitp = interpolate( sgrid, vinit, Gridded(Linear()) )
 ...
 for i = 1:sgridsize                 #sd rows
    ...
    @sync @distributed for j = 1:shockgridsize      # different matrix for shocks
        ...
        Tv[i,k,j], p = maximize_EVF(sdgrid[i], sfgrid[k], j, pd, pf, parameters, shockgrid, sdgrid, sfgrid, vinit)
        Pnd[i,k,j] = p[1]
        Pnf[i,k,j] = p[2]
      #  @show Tv[i,k,j]
