...
#@everywhere 
#V_int(sdgrid::Array{Float64}, sfgrid::Array{Float64}, col::Int64, vinit::Array{Float64}) = interpolate( (sdgrid,sfgrid), vinit[:,:,col], Gridded(Linear()) )
...
#xd_unc_min = X_UNC_min[5]
xf_unc_max = X_UNC_max[4]
#xf_unc_min = X_UNC_min[4]

#nmax::Float64  = (spmax*delta/(1-delta) + xf_unc_min)
#nmax::Float64 = (spmax/((1-delta)*(1-lam_min)))*0.9
if sf < xf_unc*(0.8)    
...
VF(n) = -value_fn(n, sd, sf, shockgrid[eta,2], lamd, shockgrid[eta,1], pd, pf, parameters, sdgrid, sfgrid, vinit)                     

results = optimize(VF, nmin, nmax, nini, Fminbox(LBFGS()), Optim.Options(f_tol = 1e-6, x_tol = 1e-6))
...
#vitp = interpolate( sgrid, vinit, Gridded(Linear()) )
 ...
 for i = 1:sgridsize                 #sd rows
    ...
    @sync @distributed for j = 1:shockgridsize      # different matrix for shocks
        ...
        Tv[i,k,j], p = maximize_EVF(sdgrid[i], sfgrid[k], j, pd, pf, lamd, parameters, shockgrid, sdgrid, sfgrid, vinit)
        Pnd[i,k,j] = p[1]
        Pnf[i,k,j] = p[2]
      #  @show Tv[i,k,j]
...
for i = 1:sgridsize                 #sd rows
    for k = 1:sgridsize             #sf column
        for j = 1:shockgridsize      # different matrix for shocks
...
@time Tv0, Pnd0, Pnf0 = Tf_d(pd, pf, lamd, parameters, shockgrid, sdgrid, sfgrid, Tv)
